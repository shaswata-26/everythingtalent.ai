    ...
    <div className="relative h-full w-full bg-gradient-to-tr from-[#fff9fd] via-[#fff6fc] to-[#ffedfa] dark:bg-gradient-to-bl dark:from-[#0e1628] dark:via-[#0a1121] dark:to-[#0e1628] py-10 sm:py-16 md:py-28 px-4 sm:px-6 lg:px-8 !pt-0">
      ...
      <div className="relative bottom-[1rem]">
        <div className="flex mt-0 relative bottom-[1rem] mx-auto items-center border rounded-full w-fit justify-center">
          <div style={{ zIndex: 45 }} className="relative group transition-transform duration-300 cursor-pointer">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-8 sm:mb-16">
          ...
          <h2 className="mt-5 mb-5 text-5xl leading-[44px] md:text-center font-bold text-gray-900 dark:text-white">
            Our Philosophy in <span className="bg-gradient-to-r from-[#ec0bc8] via-[#5bb2f6] to-[#f6c436] bg-clip-text text-transparent">Action</span>
          </h2>
...
function ValueCard({
  ...
  return (
    <div 
      style={{ zIndex: 46 }} 
      className={`relative h-full w-full overflow-hidden rounded-2xl p-4 flex flex-col ${bgColor} ${isWide ? 'sm:col-span-2' : ''}`}
    >
      ...
      {hasHoverEffect ? (
        ...
        <strong className={`mt-2 inline-block font-bold ${hasAvatars ? 'text-2xl text-white' : 'text-xl'}`}>
          {title}
        </strong>
      )}
      ...
      <AnimatedTag tag={tag} textColor="text-white" />
    </div>
  );
}
...
function TeamAvatars() {
  const teamMembers = [
    ...
    { name: "Michael Wilson", role: "Backend Developer", image: "/micheal.avif" },
    ...
    <div className="ml-4 mt-auto">
      <div className="flex py-0">
        {teamMembers.map((member, index) => (
          <div key={index} className="group relative z-0 -ml-4 flex scale-100 items-center transition-all duration-200 ease-in-out hover:z-10 hover:scale-110">
...
function SlackIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-slack w-[29px] h-[29px] rotate-180 text-white">
...
function AnimatedTag({ tag, textColor = 'text-white' }) {
  ...
  return (
    <sup className={`text-sm mt-4 ${textColor} font-bold`}>
      {displayedTag}
    </sup>
  );
}